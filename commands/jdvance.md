...
2. Check if `.jdvance/knowledge.json` exists in the current project — if so, read it. If not, ask: "Want me to create a project knowledge base?" If yes, create `.jdvance/knowledge.json` with an empty schema.
3. Read all skill files in `~/.jdvance/skills/`.
4. Begin the session start flow as described in `~/.jdvance/CLAUDE.md`.
